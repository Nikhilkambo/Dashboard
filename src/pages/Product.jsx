...
const Product = () => {
  ...
  const getProducts = async () => {
    try {
      let response = await axios.get("http://127.0.0.1:3007/api/product/list", {
        headers: { "x-auth": userDetails.token },
      });
  ...
  const deleteProduct = async (id) => {
    try {
      const response = await axios.delete(
        `http://127.0.0.1:3007/api/product/delete/${id}`,
        { headers: { "x-auth": userDetails.token } }
      );
  ...
  //       `http://127.0.0.1:3007/api/product/search?searchTerm=${searchTerm}`,
  //       { headers: { "x-auth": userDetails.token } }
  //     );
  //     let result = await response.data;
  //     if (result) {
                ...
                <tr>
                  ...
                  <td>
                    <img
                      ...
                      src={
                        item.productImage
                          ? "http://127.0.0.1:3007/api/product/" +
                            item.productImage
                  ...
                  <td>
                    <div>
                      ...
                      <Button
                        onClick={() => {
                          updateProductModalSetting(item);
                          setUpdateModel(true);
                        }}
